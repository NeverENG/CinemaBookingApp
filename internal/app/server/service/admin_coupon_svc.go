...
func (s *AdminCouponSvc) CreateTemplate(ctx context.Context, adminID int64, in CouponTemplateInput) (*domain.CouponTemplate, error) {
	...
	return tpl, s.log(ctx, adminID, "CREATE_COUPON_TEMPLATE", "coupon_template", strconv.FormatInt(tpl.ID, 10), tpl)
}

func (s *AdminCouponSvc) ListTemplates(ctx context.Context) ([]domain.CouponTemplate, error) {
	return s.coupons.ListTemplates(ctx)
}

func (s *AdminCouponSvc) SetTemplateStatus(ctx context.Context, adminID, templateID int64, status string) error {
	...
	return s.log(ctx, adminID, "SET_COUPON_TEMPLATE_STATUS", "coupon_template", strconv.FormatInt(templateID, 10), map[string]string{"status": status})
}

// IssueToUser 给指定用户发一张券。
func (s *AdminCouponSvc) IssueToUser(ctx context.Context, adminID, userID, templateID int64) (*domain.UserCoupon, error) {
	...
	err := s.tx.Run(ctx, func(txCtx context.Context) error {
		...
		return s.log(txCtx, adminID, "ISSUE_COUPON", "user", strconv.FormatInt(userID, 10), map[string]any{
