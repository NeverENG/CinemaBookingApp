...
> 目标：在不改变后端领域边界的前提下，建立胶片灰/影院红用户端、深灰管理端、分层 Router、统一 Axios 与 Server Hook 体系。
...
| --- | --- | --- |
| `USER` | `/` | 用户端 |
| `SUPER_ADMIN` | `/admin/dashboard` | 全部管理模块 |
| `CINEMA_ADMIN` | `/admin/dashboard` | 绑定影院的数据与运营模块 |
| `FINANCE` | `/admin/dashboard` | 票房大盘 |

前端守卫只负责导航和交互体验，后端 JWT + RBAC 才是最终权限边界。
...
- 积分、积分兑换优惠券。
- 管理端影片、影厅、场次、Banner、优惠券、票房大盘、管理员创建。
