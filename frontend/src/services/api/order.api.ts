...
import type { ChangeTicketResult, Order, Payment, Refund } from '../../types'
import { normalizeChangeTicketResult, normalizeOrder, normalizePayment, normalizeRefund } from './normalize'
...
export const orderApi = {
  ...
  cancel: (orderNo: string) => http.post(`/orders/${encodeURIComponent(orderNo)}/cancel`).then((response) => response.data),
  ...
  mockRefund: (refundNo: string) => http.post('/refunds/mock-callback', { refund_no: refundNo }).then((response) => response.data),
  change: async (orderNo: string, payload: { new_session_id: number; new_seat_ids: number[] }, idempotencyKey: string): Promise<ChangeTicketResult> => normalizeChangeTicketResult(await http.post(`/orders/${encodeURIComponent(orderNo)}/change`, payload, { headers: { 'Idempotency-Key': idempotencyKey } }).then((response) => response.data)),
